...
public class UserService {
    ...
    public void validateRegistration(User user, String password) throws ValidationException {
        ...
        if (user.getLogin().length() < 4) {
            throw new ValidationException("Login can't be shorter than 4 letters");
        }
    ...
    public void updateAdmin(long userId, long newValue) throws ValidationException {
        if (newValue != 0 && newValue != 1) {
            throw new ValidationException("Wrong admin value");
        }
        userRepository.updateAdmin(userId, newValue);
    }
}
