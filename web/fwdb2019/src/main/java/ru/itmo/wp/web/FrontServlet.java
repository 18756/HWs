...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        try {
            invokeMethod(findMethodByName(pageClass, "before"), page, request, view);
            invokeMethod(method, page, request, view);
            invokeMethod(findMethodByName(pageClass, "after"), page, request, view);
    ...
    private void invokeMethod(Method method, Object page, HttpServletRequest request, Map<String, Object> view) throws InvocationTargetException, IllegalAccessException, NotFoundException, ServletException {
        method.setAccessible(true);
        if (method.getParameterCount() == 0) {
            method.invoke(page);
        } else if (method.getParameterCount() == 1) {
            method.invoke(page, getParameter(method, 0, request, view));
        } else if (method.getParameterCount() == 2) {
            method.invoke(page, getParameter(method, 0, request, view),
                    getParameter(method, 1, request, view));
        } else {
            throw new ServletException("Too many parameters to method: " + method.getName());
        }
    }

    private Method findMethodByName(Class<?> clazz, String action) throws NotFoundException {
    ...
    private Object getParameter(Method method, int index, HttpServletRequest request, Map<String, Object> view) throws ServletException {
        if (method.getParameterTypes()[index] == HttpServletRequest.class) {
            return request;
        } else if (method.getParameterTypes()[index] == Map.class) {
            return view;
        } else {
            throw new ServletException("Wrong method parameter: " + method.getParameterTypes()[index].getName());
        }
    }
